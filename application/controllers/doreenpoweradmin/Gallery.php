...
class Gallery extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->_getImageSize = array(
            'original' => array('width' => 1600, 'height' => 800, 'crop' => FALSE, 'rgb' => 0xdad7d7),
            'large' => array('width' => 800, 'height' => 600, 'crop' => FALSE, 'rgb' => 0xdad7d7),
            'small' => array('width' => 400, 'height' => 300, 'crop' => FALSE, 'rgb' => 0xdad7d7),
        );
    ...
    public function index($id = false) {
        ...
        generatePagging('doreenadmin/gallery/index', $totalRow, $perPage, $uriSegment, 4);
            ...
            foreach ($allPhotos as $photo) {
                ...
                if ($photo->images) {
                    if ($this->global_model->delete('media', array('id' => $photo->id, 'type' => $this->_module))) {
                        ...
                        if (file_exists($this->_moduleImagePath . 'large/' . $photo->images)) {
                            unlink($this->_moduleImagePath . 'large/' . $photo->images);
                        }
                    }
                }
            }
    ...
    public function delete_photo($photoId, $typeId) {
        ...
        if ($this->global_model->delete('media', array('id' => $photoId))) {
            if (file_exists($this->_moduleImagePath . $item['images'])) {
                ...
                if (file_exists($this->_moduleImagePath . 'large/' . $item['images'])) {
                    unlink($this->_moduleImagePath . 'large/' . $item['images']);
                }
            }
            $this->session->set_flashdata('success_msg', 'Item deleted successfully !!');
    ...
    private function multiplePhotoUpload($imageField, $typeId = 0) {
        ...
        foreach ($allFiles as $key => $aPhoto) {
            ...
            if (isset($photo['error_msg'])) {
                ...
                $largePath = './' . $this->_moduleImagePath . 'large/' . $photo; // large image path

                img_resize($iniPath, $mainPath, $this->_getImageSize['original']);
                img_resize($iniPath, $smallPath, $this->_getImageSize['small']); // small image
                img_resize($iniPath, $largePath, $this->_getImageSize['large']); // large image
...
/* Location: ./application/controllers/doreenadmin/Gallery.php */
