...
if (!function_exists('admin_url')) {
    ...
    function admin_url($uri = '') {
        $CI = & get_instance();
        return $CI->config->base_url('doreenpoweradmin/' . $uri);
    }
...
if (!function_exists('getCoverPhoto')) {
    ...
    function getCoverPhoto($projectId, $imgSize = 'featured') {
        // get the CI instanse
        $CI = &get_instance();
        // get image
        ...
        if (!empty($mediaImage['images'])) {
            $imagePath = 'assets/media/projects/';
            $imageName = base_url($imagePath . $imgSize . '/' . $mediaImage['images']);
        } else {
            $mediaImage2 = $CI->global_model->get_data('media', array('type_id' => $projectId, 'type' => 'projects'));
          
            if (!empty($mediaImage2['images'])) {
                $imagePath = 'assets/media/projects/';
                $imageName = base_url($imagePath . $imgSize . '/' . $mediaImage2['images']);
            }
        }
        return $imageName; // return zero incase of no images.
    }
...
if (!function_exists('getCvUrl')) {
    ...
    function getCvUrl($imageName) {
        ...
        $CI = &get_instance();
        // getLink
        return base_url('assets/media/person_cv/' . $imageName);
    }
...
if (!function_exists('createPagination')) {
    ...
    function createPagination($page_url, $total_rows, $per_page, $uri_segment = 3, $num_links = 2) {
        $CI = &get_instance();
        ...
        // open full tag
        $config['full_tag_open'] = '<div class="pdb_pagination"><ul>';
        ...
        //$config['last_tag_open'] = '<a href="" class="last">';
...
if (!function_exists('getMediaAll')) {
    ...
    function getMediaAll($table, $id) {
        ...
        $CI = &get_instance();
        //Get image
        $results = $CI->global_model->get($table, array('type_id' => $id));
        return $results;
    }
...
if (!function_exists('readStatus')) {
    ...
    function readStatus($status) {
        if ($status == 'read') {
            ...
            return '<span class="action label label-danger">Unread</span>';
        }
    }
...
if (!function_exists('getBanners')) {
    ...
    function getBanners() {
        $CI = &get_instance();
        $getBanners = $CI->global_model->getAllData("banners");
        if (!empty($getBanners)):
            return $getBanners;
        endif;
...
if (!function_exists('getCategoryDropDown')) {
    ...
    function getCategoryDropDown() {
        $CI = &get_instance();
        $categories = $CI->global_model->get('project_category', array('category_status' => 1));
        $response = array('' => '-- Choose Category --');
        if (!empty($categories) && count($categories) != 0) {
            foreach ($categories as $row) {
                $response[$row->project_category_id] = $row->category_name;
            }
            return $response;
        }
...
if (!function_exists('getProjectNameById')) {
    ...
    function getProjectNameById($id) {
        $CI = &get_instance();
        $projects = $CI->global_model->getData('projects', array('project_id' => $id));
        if (!empty($projects)) {
...
if (!function_exists('getProjectImages')) {
    ...
    function getProjectImages($projectId, $type = 'projects', $size = 'small') {
...
if (!function_exists('getAllProjectInSearch')) {
    ...
    function getAllProjectInSearch() {
        ...
        $projects = $CI->projects_model->getProjectsBySearch();
...
if (!function_exists('countAlbumPhoto')) {
    ...
    function countAlbumPhoto($albumId) {
        ...
        $CI = &get_instance();
        $count = $CI->global_model->countAlbumPhoto($albumId);
        return $count;
    }
